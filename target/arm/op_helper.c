...
extern void offload_server_send_cmpxchg_start(uint32_t);
extern void offload_server_send_cmpxchg_end(uint32_t);
pthread_mutex_t cmpxchg_mutex = PTHREAD_MUTEX_INITIALIZER;
void HELPER(offload_cmpxchg_prelude)(uint32_t addr)
{
	fprintf(stderr, "helper_offload_cmpxchg_prelude\n");
	//pthread_mutex_lock(&cmpxchg_mutex);

	offload_server_send_cmpxchg_start((((uint32_t)addr) / 0x1000) * 0x1000);
    printf(stderr, "after prelude, addr: %x\n", addr);
	fprintf(stderr, "after prelude, val: %x\n", *((uint32_t *)(g2h(addr))));

    mprotect(g2h((((uint32_t)addr) / 0x1000) * 0x1000), TARGET_PAGE_SIZE, PROT_READ | PROT_WRITE);
}



void HELPER(offload_cmpxchg_epilogue)(uint32_t addr)
{
