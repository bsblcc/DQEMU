...
__thread int last_flag_recv = 1; // whether we received something.
__thread int last_flag_pending = 1; // whether we had a pending request.
__thread int last_flag_lock = 1; // whether we succeeded on try_lock
...
static void offload_client_daemonize(void)
{
	...
	while (1)
	{
        ...
        //offload_log(stderr, "hpr: %d, type: %d\n", has_pending_request, pending_request.type);
		if (has_pending_request != 0)
		{
			...
			if (pending_request.type == 1)
			{
                ...
                if (last_flag_lock == 1)
    			{
                    ...
                    if (requestor == -1)
                    {
                        // futex
                        //has_pending_request = 1;
                        pending_request.type = 2;
                    }
                    else if (requestor >= 0)
                    {
                        //offload_log(stderr, "set hpr to 0\n");
                        has_pending_request = 0;
			...
			if (pending_request.type == 2)
            {
                ...
                res = pthread_mutex_trylock(&pmd->recv_mutex);
                if (res == 0)
                {
                    //offload_log(stderr, "[offload_client_daemonize]\tpending try lock succeed\n");

                    if (pmd->invalid_count == 0)
                    {
                        //offload_log(stderr, "[offload_client_daemonize]\tpending futex wait\n");

                        int ret = offload_client_futex_wait(pending_request.uaddr,
                ...
                {
                    //offload_log(stderr, "[offload_client_daemonize]\tpending try lock failed\n");
                    //has_pending_request = 1;
                    //pending_request.type = 2;
                }
            }
...
static void offload_process_page_content(void)
{
   ...
   // char *payload = net_buffer + sizeof(struct tcp_msg_header);
	...
	if (perm == 2)
	{
        fprintf(stderr, "[offload_process_page_content]\tinvalid count: %d\n", pmd->invalid_count);
		...
		if (pmd->invalid_count - 1 == 0)
		{
			...
			else if (requestor_idx == -1)
			{
                ...
                offload_log(stderr, "mprotect page: %x\n", page_addr);
...
static void offload_client_process_futex_wait_request()
{
    ...
    has_pending_request = 1;
    pending_request.type = 1;
    pending_request.requestor = -1;
...
static void offload_client_process_futex_wake_request()
{
    ...
    int ret = offload_client_futex_wake(uaddr, op, val, timeout, uaddr2, val3);
...
static int offload_client_futex_prelude(target_ulong page_addr)
{
	...
	if (ret == 0)
    {
        ...
        // we can wait here.
        ret = pthread_mutex_trylock(&pmd->recv_mutex);
        if (ret == 0)
        {
            if (pmd->invalid_count == 0)
            {
